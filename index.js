...
app.get("/addpuppies10", function(request,response) {
	puppies=puppies+10;
	response.render('pages/index')
});
app.get("/addpuppies100", function(request,response) {
	puppies=puppies+100;
	response.render('pages/index')
});
app.get("/clearpuppies",function(request,response) {
	puppies=-0;
...
app.get('/test', function(request, response) {
	...
	console.log(request.query.puppies);
	...
	pg.connect(process.env.DATABASE_URL, function(err, client, done) {
		...
		client.query('SELECT * FROM ballots', function(err, result) {
			...
			if (err){ 
				...
				votes=result.rows;
				for (z=0;z<puppies;z++) {
					votes = votes.concat(poop);
				}
				for (x=0;x<votes.length;x++) {
					nominee1=votes[x].work.trim();
					match=0;
// loop through nomination list looking for current nominee
// if found increment vote count and increment match to show it was found
...
// if not found add to nomination list
					if (match==0) {
						   newnomline={};
						   newnomline.points=1;
...
//sort nominees based on points
				...
				console.log (winners.length+ "Winners length "+ categorycount[1]);
...
app.get('/fourandsix', function (request, response) {
//	winners.length=0;
	pg.connect(process.env.DATABASE_URL, function(err, client, done) {
		nomlist.length=0;
		client.query('SELECT * FROM ballots where rank<5', function(err, result) {
			done();
			if (err){ 
				console.error(err); response.send("Error " + err);
			}
...
app.get('/eph', function (request, response) {
	blanknom={};
	pg.connect(process.env.DATABASE_URL, function(err, client, done) {
		client.query('SELECT * FROM ballots', function(err, result) {
			done();
